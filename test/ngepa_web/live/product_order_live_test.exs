defmodule NgepaWeb.ProductOrderLiveTest do
  ...
  describe "Index" do
    ...
    test "updates product_order in listing", %{conn: conn, product_order: product_order} do
      ...
      assert index_live
             |> element("#product_order-#{product_order.id} a", "Edit")
             |> render_click() =~
               "Edit Product order"
    ...
    test "deletes product_order in listing", %{conn: conn, product_order: product_order} do
      ...
      assert index_live
             |> element("#product_order-#{product_order.id} a", "Delete")
             |> render_click()
  ...
  describe "Show" do
    ...
    test "displays product_order", %{conn: conn, product_order: product_order} do
      {:ok, _show_live, html} =
        live(conn, Routes.product_order_show_path(conn, :show, product_order))
